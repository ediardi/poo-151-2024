...
MapGenerator::MapGenerator(unsigned int max_x, unsigned int max_y, const int nr_nodes, const int minradc) :
        nr_nodes(nr_nodes),minrad(minradc),player(Capturer(sf::Color::Cyan,sf::Color::Blue)),enemy(Capturer(sf::Color::Magenta,sf::Color::Red)){
    Node::set_rad(minrad);
...
bool MapGenerator::action(float x, float y) {
    for(Node* node:v)
    {
